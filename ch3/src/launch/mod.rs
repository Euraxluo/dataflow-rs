...
/// 根据描述文件启动所有的节点
pub async fn launch(dataflow: PathBuf) -> Result<()> {
    // 读取描述文件并解析
    ...
    // 启动所有的节点
    launch_nodes(&nodes, &descriptor, &working_dir).await?;
    Ok(())
}
...
async fn launch_nodes(
    ...
    debug!("Launch Nodes");
    timer::start(&nodes, &descriptor.deploy.endpoints.clone().unwrap()).await?;

    let mut tasks = FuturesUnordered::new();
    for node in nodes {
        let node_id = node.id.clone();
        let result = spawn_node(node.clone(), descriptor, working_dir)
            .await
            .with_context(|| format!("failed to spawn runtime node {node_id}"))?;
        tasks.push(result);
    }
    ...
    while let Some(task_result) = tasks.next().await {
        task_result
            ...
            .context("launch node failed")?;
    }

    debug!("Launch Success");
    Ok(())
}
...
async fn spawn_node(
    ...
    command.args(["start", "--node", node.id.as_str()]);
